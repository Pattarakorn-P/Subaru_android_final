...
class MainChart : Fragment() {
    ...
    fun Bar_Chart( chart : BarChart ){
        ...
        //สุ่มข้อมูล 12 อัน
        val listSubaru = Sti.getSampleStiData(12)

        //สร้าง ArrayList<BarEntry> ขึ้นมาเพื่อเก็บข้อมูลการขาย และใส่ข้อมูล ลงไปใน entry แต่ละตัว
        val entries: ArrayList<BarEntry> = ArrayList()
        var index : Float = 0F
        for (subaru in listSubaru) {
            entries.add(BarEntry(index, subaru.sales))
            index++
        }
...
//        xAxis.setCenterAxisLabels(false)
        ...
        xAxis.setValueFormatter(object : ValueFormatter() {
            ...
            override fun getFormattedValue(value: Float): String? {
                ...
                if( value < 0 || value >= listSubaru.size ){
                    return ""
                }
                else{
                    return listSubaru[value.toInt()].name
                }
...
//        rightAxis.axisMinimum = 0f // this replaces setStartAtZero(true)
    ...
    fun Pie_Chart( chart: PieChart){
        ...
        //สุ่มข้อมูล 5 อัน
        val listSubaru = Sti.getSampleStiData(5)

        val entries: ArrayList<PieEntry> = ArrayList()
        for (subaru in listSubaru) {
            entries.add(PieEntry(subaru.sales, subaru.name))
        }

        val dataset = PieDataSet(entries, "Subaru")
